...
def Kbundle_QP_relaxation(mu_vec, Cov_mat, alpha, K, separate_profit_vec, mandatory_products):
	N = len(mu_vec)
	...
	for M in range(K, min(3*K+1, N), max(2*K//chunk,1)):
		...
		opt_soln = Kbundle_candidate(sorted_indices[:M], Cov_mat, K, len(mandatory_products))
...
def Kbundle_candidate(candidate_list, Cov_mat, K, N_mandatory_products):
	Cov_mat_sub = Cov_mat[:,candidate_list][candidate_list]

	total_product = Cov_mat.shape[0]

	b = np.zeros(len(candidate_list))
	c = np.zeros(len(b))
	for i in range(N_mandatory_products):
		c[i] = 1.


	def ObjectiveFunction(b):
		return b.dot(Cov_mat_sub).dot(b)

	constraints = [{'type': 'eq', 'fun': lambda b: sum(b) - K}]
	for i in np.where(c == 1.):
		constraints.append({'type': 'eq', 'fun': lambda b: b[i] - 1})
	bounds = Bounds(lb=0, ub=1)

	result = minimize(ObjectiveFunction, b, bounds=bounds, method='SLSQP', constraints=constraints, options={'disp': False})
