...
def evaluate_prediction(predicted_cov_mat, copurchase_mat, mean_vec, price_vec, std_vec, sales_vec, delta_vec):
	...
	total_rank = np.dot(rank_mat, copurchase_mat).sum()
...
if __name__ == '__main__':
	...
	copurchase_mat = pd.read_csv('data/matrix_correlacao.csv', delimiter=',', header=None).as_matrix()
	...
	products = json.load(open('data/products_list.json'))
	...
	with open('results/result_accuracy_' + str(20) + '.json', 'w') as output_file:
